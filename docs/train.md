...
| -Z FINE_TUNE_FREEZE_EPOCHS, --fine-tune-freeze-epochs FINE_TUNE_FREEZE_EPOCHS | number of freeze epochs to train in second round. See https://docs.fast.ai/callback.schedule.html#learner.fine_tune (default: 5) |
| -E FINE_TUNE_EPOCHS, --fine-tune-epochs FINE_TUNE_EPOCHS | number of freeze epochs to train in second round. See https://docs.fast.ai/callback.schedule.html#learner.fine_tune (default: 0) |
...
By default, varKoder will implement two rounds of traning. `-e` and `-z` set the number of epochs with the learning rate given with `-r`, and `-E` and `-Z` will set the number of additional epochs with a lower learning rate (20 times lower, to be more precise).
