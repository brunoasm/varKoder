...
| `-p KMER_MAPPING`, `--kmer-mapping KMER_MAPPING` | method to map kmers to pixels. This sets the correspondence between specific kmers and coordinates in the image produced. The possible values are: `varKode` (default), which produces varKodes; `cgr`, which produces a chaos game representation. See the [convert command documentation](convert.md) for more details.|
